...
/**
 * Define a global function to preserve `Normalized_` type information when saving a prebuilt L2-normalized index in `knncolle::Prebuilt::save()`.
 ...
 * It is recommended that this file should not start with an upper-case letter to avoid conflicts with files generated by `save()`.
 * 
 ...
 * If set, the global function will be called by the `knncolle::Prebuilt::save()` method for the L2-normalized `Prebuilt` subclass.
 */
template<typename Normalized_>
std::function<void(const std::string&)>& custom_save_for_l2normalized_normalized() {
